...
# ---------------- Data audit logic (from your advanced audit) ----------------
...
# ---------------- Helper for feature preparation (simple) ----------------
def prepare_features_for_training(df):
    # Drop identifiers, keep numeric and one-hot encode categoricals
    X = df.drop(columns=['building_id'], errors='ignore').copy()
    if 'price' in X.columns:
        X = X.drop(columns=['price'])
    # Convert boolean/int categories as-is
    cat_cols = X.select_dtypes(include=['object', 'category']).columns.tolist()
    X = pd.get_dummies(X, columns=cat_cols, drop_first=True)
    return X

# ---------------- Main GUI app (with front page) ----------------
class MergedHousePriceApp:
    def __init__(self, root):
        self.root = root
        self.root.title("House Price Prediction Dashboard - Merged")
        ...
        # create notebook but first show front page
    ...
    def _run_cli_tests(self):
        # Run the same tests you had in CLI, but pop up results
        try:
            rc = self._run_tests_internal()
    ...
    def _show_preview(self):
        ...
        if self.df is None:
            ...
            return
        txt = tk.Text(self.data_preview_frame, height=20)
        txt.pack(fill='both', expand=True)
        txt.insert('1.0', f'Rows: {len(self.df)}  Columns: {len(self.df.columns)}\n\n')
        txt.insert('end', self.df.head(20).to_string(index=False))
    ...
    def _build_predict_tab(self):
        ...
        fields = ['area_sqft', 'bedrooms', 'bathrooms', 'total_floors', 'floor_number', 'age_years', 'parking_spaces', 'balconies', 'nearby_schools', 'nearby_hospitals', 'city_district', 'property_type', 'furnishing_status']
        for f_name in ['area_sqft','bedrooms','bathrooms','age_years','parking_spaces','balconies','nearby_schools','nearby_hospitals']:
            ttk.Label(left, text=f_name).pack(); ent = ttk.Entry(left); ent.pack(); self.predict_entries[f_name]=ent
        # small dropdowns for categorical
